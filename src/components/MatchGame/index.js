...
class MatchGame extends Component {
  ...
  clearTimer = () => clearInterval(this.timeInterval)
  ...
  clickPhoto = id => {
    ...
    if (id === activeImgId) {
      ...
      this.setState({
        ...
        activeImgUrl: imagesList[randomNum].imageUrl,
      })
    } else {
      this.setState({isGameInProgress: false})
      this.clearTimer()
  ...
  renderImages = () => {
    const {activeTabId, activeImgUrl} = this.state
    const getFilteredImages = this.getActiveTabItems(imagesList)
    return (
      <div className="game-body">
        <img src={activeImgUrl} alt="match" className="image-display" />
        ...
        <ul className="images-list">
          {getFilteredImages.map(eachImage => (
            <PhotoItem
              key={eachImage.id}
  ...
  render() {
    const {isGameInProgress, score, timeRemaining} = this.state

    return (
      <div className="app-container">
        <NavBar score={score} timeRemaining={timeRemaining} />
        <div className="match-game-body">
          {isGameInProgress ? this.renderImages() : this.renderScoreCard()}
        </div>
      </div>
    )
  }
